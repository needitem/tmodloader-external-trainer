...
///
/// CLIENT-side on purpose: the shop the host sees is built from the client's own <c>Main.travelShop</c>
/// (in MP the host's view never reads the server's copy), so the re-roll must run here. The merchant's
/// physical arrival is a separate, server-authoritative action — see <see cref="TravelMerchantSpawner"/>.
/// Re-resolves addresses each click to survive tiered-JIT relocation.
/// </summary>
