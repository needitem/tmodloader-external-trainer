...
public enum RowKind { GroupHeader, Value, Toggle, Buff, Action, Inject, Fast, UseHook, Tools, Craft, Patch, Vanity, PatchSet, DropMult, Crate, ScopedDrop, BuffClear, PatchInt, InfAmmo, ZoneForce }
...
public static class CheatTable
{
    public static List<CheatRow> Build(TmlModel model, IReadOnlyList<BuffCheat> buffs, CodeInjector? injector = null)
    {
        ...
        foreach (var d in dtos)
        {
            ...
            if (d.Kind.Equals("zone", StringComparison.OrdinalIgnoreCase))
            {
                EmitGroup(rows, ref lastGroup, d.Group);
                rows.Add(new CheatRow { Kind = RowKind.ZoneForce, Group = d.Group, Desc = d.Desc, InjectValue = d.Value ?? "0:0" });
                continue;
            }
